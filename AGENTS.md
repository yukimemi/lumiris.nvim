...
The sections above the marker blocks are lumiris-specific and
...
rtp 上の**古いコピー**を読んでテストが通ってしまう事故が起きる。mini.test +
`-u NONE -l` はこれを構造的に回避する。
...
- 対話的に回すとき: `nvim --noplugin -u scripts/minitest_init.lua` → `:lua MiniTest.run()`
